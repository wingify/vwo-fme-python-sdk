...
from wingify.services.batch_event_queue import BatchEventQueue
...
class WingifyClient:
    """
    Canonical FME SDK client. All feature evaluation, tracking, and attribute APIs
    are implemented here. Legacy VWO integrations use VWOClient (alias) from vwo_client.py.
    """
    ...
    def __init__(self, settings: str, options: Dict):
        ...
        # Singleton — one active client per process
        if WingifyClient._client_instance is None:
            WingifyClient._client_instance = self
    ...
    @staticmethod
    def get_instance():
        """Return the singleton WingifyClient instance, if initialized."""
        return WingifyClient._client_instance
