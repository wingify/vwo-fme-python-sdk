...
def get_event_batching_query_params(account_id: str) -> Dict[str, Any]:
    return {"a": account_id, "sd": _get_sdk_name(), "sv": Constants.SDK_VERSION}
...
def send_post_api_request(
    ...
    try:
        ...
        if event_name == EventEnum.VARIATION_SHOWN.value:
        ...
        elif event_name == EventEnum.SYNC_VISITOR_PROP.value:
            api_name = ApiEnum.SET_ATTRIBUTE.value
            extra_data_for_message = api_name
        elif event_name != EventEnum.LOG_EVENT.value or event_name != EventEnum.DEBUGGER_EVENT.value or event_name != EventEnum.SDK_INIT_EVENT.value or event_name != EventEnum.USAGE_STATS.value:
            api_name = ApiEnum.TRACK_EVENT.value
            extra_data_for_message = api_name
        
        # Create the request model
        is_gateway_service = SettingsManager.get_instance().is_gateway_service_provided
        request = RequestModel(
            SettingsManager.get_instance().get_events_hostname(),
            "POST",
            UrlService.get_endpoint_with_collection_prefix(
                endpoint=UrlEnum.EVENTS.value, is_gateway_service=is_gateway_service
            ),
            properties,
            payload,
            headers,
...
def send_post_batch_request(
    ...
    try:
        ...
        request_model = RequestModel(
            ...
            "POST",
            UrlService.get_endpoint_with_collection_prefix(
                endpoint=UrlEnum.BATCH_EVENTS.value, is_gateway_service=is_gateway_service
            ),
            query,
            batch_payload,
            {
...
def send_event(
    ...
    try:
        base_url = SettingsManager.get_instance().get_events_hostname()
...
def get_debugger_event_payload(event_props: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = _get_event_base_payload(None, account_id + "_" + sdk_key, EventEnum.DEBUGGER_EVENT.value)
    ...
    payload["d"]["event"]["props"]["vwoMeta"]["sn"] = _get_sdk_name()
