...
def get_from_gateway_service(query_params: Dict[str, Any], endpoint: str, context: ContextModel) -> Any:
    ...
    try:
        ...
        request = RequestModel(
            url=SettingsManager.get_instance().get_settings_hostname(),
            method="GET",
            path=UrlService.get_endpoint_with_collection_prefix(
                endpoint=endpoint, is_gateway_service=is_gateway_service
            ),
            query=query_params,
...
def post_to_gateway_service(query_params: Dict[str, Any], payload: Dict[str, Any], endpoint: str) -> Any:
    ...
    try:
        ...
        request = RequestModel(
            url=SettingsManager.get_instance().get_settings_hostname(),
            method="POST",
            path=UrlService.get_endpoint_with_collection_prefix(endpoint=endpoint, is_gateway_service=is_gateway_service),
            query=query_params,
            body=payload,
            scheme=SettingsManager.get_instance().protocol,
            port=SettingsManager.get_instance().port,
