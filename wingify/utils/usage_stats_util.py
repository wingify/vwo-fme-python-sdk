...
class UsageStatsUtil:
    ...
    def set_usage_stats(self, options: Dict[str, Any]) -> None:
        ...
        # Check for SDK meta event-analytics flag (_wingify_meta or legacy _vwo_meta)
        sdk_meta = options.get("_wingify_meta") or options.get("_vwo_meta")
        if sdk_meta and sdk_meta.get("ea"):
            data["_ea"] = 1
