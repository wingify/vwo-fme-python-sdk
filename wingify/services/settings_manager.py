...
class SettingsManager:
    ...
    def __init__(self, options):
        ...
        self.proxy_url = None
        self.is_via_vwo = bool(options.get("is_via_vwo", False))
        self.sdk_name = get_sdk_name(self.is_via_vwo)

        # Hosts depend on init path: vwo.init() vs wingify.init()
        self.settings_hostname = get_settings_hostname(self.is_via_vwo)
        self.events_hostname = get_events_hostname(self.is_via_vwo)
        self.protocol = Constants.HTTPS_PROTOCOL
        self.port = None

        # Backward-compatible alias — settings traffic uses settings_hostname
        self.hostname = self.settings_hostname
        ...
        if ("proxy_url" in options and options["proxy_url"] is not None ) and not self.is_gateway_service_provided:
            ...
            self.settings_hostname = parsed_url.hostname
            self.events_hostname = parsed_url.hostname
            self.hostname = self.settings_hostname
    ...
    def get_settings_hostname(self):
        """Host used for settings, pull, location, and gateway requests."""
        return self.settings_hostname

    def get_events_hostname(self):
        """Host used for event and batch-event requests."""
        return self.events_hostname

    def is_on_platform_host(self) -> bool:
        """
        True when requests target the baked-in platform host (not a custom proxy/gateway).
        Used to decide whether segmentation should call the gateway service.
        """
        host = self.settings_hostname or ""
        return (
            Constants.VWO_HOST_NAME in host
            or Constants.WINGIFY_SETTINGS_HOST_NAME in host
            or Constants.WINGIFY_COLLECTION_HOST_NAME in host
        )
