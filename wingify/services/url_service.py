...
class UrlService:
    ...
    @staticmethod
    def get_endpoint_with_collection_prefix(endpoint: str, is_gateway_service: bool = False):
        """
        Retrieves the endpoint with collection prefix.
        """
        if is_gateway_service:
            return f"{endpoint}"
        elif hasattr(UrlService, "collection_prefix") and UrlService.collection_prefix:
