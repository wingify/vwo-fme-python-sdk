...
from .wingify_client import WingifyClient
import random
import json
import threading
...
class WingifyBuilder:
    """
    Canonical SDK builder. Wires logger, settings, storage, network, segmentation,
    polling, and batching before producing a WingifyClient instance.
    """
    ...
    def init_batching(self):
        ...
        if (
            ...
            if "batch_event_data" in self.options and self.options["batch_event_data"] is not None:
                ...
                self.client_instance.batch_event_queue = self.batch_event_queue
    ...
    def build(self, settings):
        self.client_instance = WingifyClient(settings, self.options)
            ...
            try:
                ...
                if latest_settings and json.dumps(
                    ...
                    set_settings_and_add_campaigns_to_rules(
                        latest_settings, self.client_instance
                    )
