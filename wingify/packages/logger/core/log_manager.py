...
class LogManager(Logger):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        is_via_vwo = get_is_via_vwo()
        self.name = self.config.get("name", f"{get_brand(is_via_vwo)} Logger")
        self.request_id = self.config.get("requestId", "unique-request-id")
        self.level = self.config.get("level", LogLevelEnum.ERROR)
        self.prefix = self.config.get("prefix", get_log_prefix(is_via_vwo))
    ...
    def error_log(self, template: str, data: Dict[str, Any] = {}, debug_data: Dict[str, Any] = {}, should_send_log_to_vwo: bool = True) -> None:
        try:
            ...
            print(f"[{get_log_prefix(get_is_via_vwo())}]: Error logging message: {e}")
