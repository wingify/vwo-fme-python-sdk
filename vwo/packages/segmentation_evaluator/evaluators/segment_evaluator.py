...
class SegmentEvaluator:
        ...
        for dsl in dsl_nodes:
                ...
                if key == SegmentOperatorValueEnum.FEATURE_ID.value:
                    ...
                    if feature_id_value == "on" or feature_id_value == "off":
                        ...
                        if feature:
                            ...
                            LogManager.get_instance().error_log("FEATURE_NOT_FOUND_WITH_ID",data={"featureId": feature_id_key}, debug_data={"an": ApiEnum.GET_FLAG.value, "uuid": self.context.get_vwo_uuid(), "sId": self.context.get_session_id()})
                            return None
            ...
            if is_ua_parser and key_count == len(dsl_nodes):
                try:
                    ...
                    LogManager.get_instance().error_log("USER_AGENT_VALIDATION_ERROR",data={"err": str(err)}, debug_data={"an": ApiEnum.GET_FLAG.value, "uuid": self.context.get_vwo_uuid(), "sId": self.context.get_session_id()})
    ...
    def check_location_pre_segmentation(self, location_map: Dict[str, Any]):
        if self.context.get_ip_address() is None:
            LogManager.get_instance().error_log("INVALID_IP_ADDRESS_IN_CONTEXT_FOR_PRE_SEGMENTATION", debug_data={"an": ApiEnum.GET_FLAG.value, "uuid": self.context.get_vwo_uuid(), "sId": self.context.get_session_id()})
            return False
    ...
    def check_user_agent_parser(self, ua_parser_map: Dict[str, List[str]]):
        if not self.context.get_user_agent():
            LogManager.get_instance().error_log("INVALID_USER_AGENT_IN_CONTEXT_FOR_PRE_SEGMENTATION", debug_data={"an": ApiEnum.GET_FLAG.value, "uuid": self.context.get_vwo_uuid(), "sId": self.context.get_session_id()})
            return False
